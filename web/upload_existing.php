...
echo '<h1 class="page-head-line">Upload an Existing File to Database</h1>';
...
if (isset($_GET['error']) || isset($_GET['success']))
{
	...
	elseif ($_GET['error']=="existingLoanNum")
		echo '<div class="alert alert-danger" role="alert">Loan Number already exists, reenter on this page!</div>';
...
if(isset($_POST['submit']) && $_POST['submit']=="submit")
{
	...
	{
		redirect("upload_existing.php?error=FileMimeInvalid");
	}
	else{
		$now=date("Y-m-d H:i:s");
		$autoID=$_POST['loanNum']; 
		$loanNumber = getLoanNumber($autoID);
		if($loanNumber == NULL){
			redirect("upload_existing.php?error=loanNumNull");
		}
    ...
    default:
		...
		insertFileDataToDatabase([$loanNumber, $fileSize, $now, $contentsClean, 'manual', $now, $docType, $autoID]);
		redirect("upload_existing.php?success=FileUploaded");
